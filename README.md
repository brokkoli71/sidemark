...
- **Open PowerPoints directly — and present on them** — drop a `.pptx` in and it's imported as an editable *Sidemark Deck*: each slide comes in looking pixel-identical (rendered via LibreOffice) with its **speaker notes in the notes panel**, and from there you can reorder slides, annotate with ink or text boxes, present with `F5` (live ink on a second screen), and export back to PDF
...
- **Create presentations too** — *Sidemark Deck* is a built-in slide editor: 16:9 slides with text boxes, images, alignment snapping and quick-start layouts, saved as a plain-JSON `.smdeck` file and exported to a vector PDF (☰ **New presentation**, `Ctrl+Alt+P`, or `sidemark --presentation`)
...
- **Import a PowerPoint** — open a `.pptx` and it becomes an editable deck: LibreOffice renders each slide (so it looks exactly right) into a full-slide picture, speaker notes come in on each slide, and the file's **theme** (background, fonts, colors and title/body layout) is imported too, so any slides you add afterwards match the original design. You can then reorder, annotate, present and export like any deck. The slides are pictures, so the original text isn't editable as text (yet); requires the optional LibreOffice backend
- **Themes** — every deck has a theme (page background, fonts, heading/body colors and an accent); pick one from the **theme** button in the header (a light **Classic** and a hand-tuned dark **Midnight**). New slides come out in the deck's theme, and the whole deck restyles instantly (one `Ctrl+Z` undoes it) — text boxes keep their place, only the look changes
