...
- **Share to phone** — **Share to phone…** in the ☰ menu shows a QR code (and link) that opens the current PDF in your phone's browser. It spins up a one-shot HTTP server on your LAN serving just that file under a random path, stopped when you close the dialog — both devices must be on the same Wi-Fi. The QR needs the optional [`qrencode`](https://fukuchi.org/works/qrencode/) tool (`pacman -S qrencode`); without it the link is shown as text
