...
- **Import a PowerPoint** — open a `.pptx` and it becomes an editable deck: LibreOffice renders each slide (so it looks exactly right) into a full-slide picture, speaker notes come in on each slide, and the file's **theme** (background — a solid colour *or* a full-bleed picture background — fonts, colors and title/body layout) is imported too, so any slides you add afterwards match the original design. You can then reorder, annotate, present and export like any deck. The slides are pictures, so the original text isn't editable as text (yet); requires the optional LibreOffice backend. (Fonts render faithfully only if the source typeface is installed on your system.)
