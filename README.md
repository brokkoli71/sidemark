...
- **Import a PowerPoint** — open a `.pptx` and it becomes an editable deck: LibreOffice renders each slide (so it looks exactly right) into a full-slide picture, speaker notes come in on each slide, and you can then reorder, annotate, present and export like any deck. The slides are pictures, so the original text isn't editable as text (yet); requires the optional LibreOffice backend
